...
import { setPassword } from '../../src/models/User.js';
...
test('a password-reset token can only be consumed once under concurrent requests', async (t) => {
  ...
  const results = await Promise.all([consumePasswordResetToken('hashed-reset-token'), consumePasswordResetToken('hashed-reset-token')]);
  assert.equal(results.filter(Boolean).length, 1);
  assert.equal(await consumePasswordResetToken('hashed-reset-token'), null);
});

test('wellness password writes retire all older unused setup/reset links for that user', async (t) => {
  const calls = [];
  t.mock.method(pool, 'query', async (sql, params) => { calls.push({ sql, params }); return sql.startsWith('SELECT') ? [[{ id: 'user' }]] : [{ affectedRows: 1 }]; });
  await setPassword('user', { passwordHash: 'new-password-hash', mustChangePassword: false });
  assert.match(calls[0].sql, /^UPDATE password_reset_tokens .*WHERE user_id = \? AND used_at IS NULL$/);
  assert.deepEqual(calls[0].params, ['user']);
  assert.match(calls[1].sql, /^UPDATE users SET password_hash/);
});
...
test('two distinct valid reset links cannot both replace the account password', async (t) => {
  ...
  const results = await Promise.all([resetPasswordWithToken(createHash('sha256').update('token-a').digest('hex'), 'first-password'), resetPasswordWithToken(createHash('sha256').update('token-b').digest('hex'), 'second-password')]);
  assert.equal(results.filter(Boolean).length, 1);
...
test('password, reset-token retirement and session revocation roll back together on failure', async (t) => {
  ...
  await assert.rejects(resetPasswordWithToken(createHash('sha256').update('token-a').digest('hex'), 'first-password'), /simulated storage failure/);
