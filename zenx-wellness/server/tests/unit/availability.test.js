...
test('a Kolkata Wednesday block is unavailable on Tuesday in America/Chicago', () => {
  // Wednesday 2026-09-09 09:00–11:00 Asia/Kolkata = 03:30–05:30 UTC = Tuesday 22:30–Wednesday 00:30 CDT.
  const timezone = 'Asia/Kolkata';
  const workingHours = [
    { weekday: 2, startTime: '09:00', endTime: '17:00' },
    { weekday: 3, startTime: '09:00', endTime: '17:00' },
  ];
  const exceptions = [{ startAt: '2026-09-09T03:30:00.000Z', endAt: '2026-09-09T05:30:00.000Z', kind: 'closed' }];
  // Tuesday Chicago ends at 05:00 UTC Wednesday; the last Tuesday slot is 04:30 UTC (11:30 PM CDT).
  const tuesdayBlockedStarts = ['2026-09-09T03:30:00.000Z', '2026-09-09T04:00:00.000Z', '2026-09-09T04:30:00.000Z'];
  const wednesdayBlockedStart = '2026-09-09T05:00:00.000Z';

  const openTuesday = listAvailableSlots({
    workingHours,
    date: '2026-09-08',
    timezone,
    dateTimezone: 'America/Chicago',
  });
  for (const iso of tuesdayBlockedStarts) {
    assert.ok(openTuesday.includes(iso), `expected ${iso} to be listed on Chicago Tuesday when unblocked`);
  }

  const blockedTuesday = listAvailableSlots({
    workingHours,
    exceptions,
    date: '2026-09-08',
    timezone,
  ...
  for (const iso of tuesdayBlockedStarts) {
    assert.ok(!blockedTuesday.includes(iso), `blocked Kolkata Wednesday slot ${iso} must not appear as free on Chicago Tuesday`);
  }

  const blockedWednesday = listAvailableSlots({
    workingHours,
    exceptions,
    date: '2026-09-09',
    timezone,
  ...
  assert.ok(
    !blockedWednesday.includes(wednesdayBlockedStart),
    'the remainder of the block after Chicago midnight must stay unavailable on Wednesday'
  );
});
