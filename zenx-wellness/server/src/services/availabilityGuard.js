...
import { CALL_DURATION_MINUTES, calendarDayRange, checkAvailability, listAvailableSlots } from './availability.js';
...
// in `dateTimezone` (the viewer's zone). When omitted, dateTimezone falls back to the dietitian's
// zone so older callers keep dietitian-local days.
export async function getAvailableSlotsForDay({ dietitianId, date, excludeCallId, dateTimezone }, conn = pool) {
  const timezone = await getDietitianTimezone(dietitianId, conn);
  const dayZone = dateTimezone || timezone;
  const { dayStart, dayEnd } = calendarDayRange(date, dayZone);
  // Widen the fetch by one slot-duration on each side so a call that starts just before this
  // viewer's midnight (and so overlaps this day's first candidate slot) or just after their day
  // ends isn't missed — listAvailableSlots's per-candidate overlap check still only counts a call
  // that actually overlaps a given candidate, so the extra rows are harmless.
