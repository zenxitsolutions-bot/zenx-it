...
// Enumerates every bookable slot start on one calendar day *in the dietitian's timezone*, reusing
// checkAvailability(...) as the single source of truth — a slot is listed here if and only if
// booking it would actually succeed, so "shown as available" can never drift from "actually
// bookable" at the moment of viewing. date: 'YYYY-MM-DD', interpreted as the dietitian's local
// calendar date (not a UTC one — see the module comment for why). Returns UTC ISO datetime strings;
// converting those to whatever the *viewer* needs to see is the display boundary's job, not this
// function's (see client/src/components/portal/shared/SlotPicker.jsx).
export function listAvailableSlots({
  ...
  // fromZonedTime resolves "midnight on this calendar date, in this IANA zone" to the real UTC
  // instant it corresponds to — DST-aware (a local day can be 23 or 25 UTC hours on a transition
  // day), which is exactly why this isn't `dayStart + 24h`.
  const dayStart = fromZonedTime(`${date}T00:00:00`, timezone);
  const dayEnd = fromZonedTime(`${nextDateString(date)}T00:00:00`, timezone);
