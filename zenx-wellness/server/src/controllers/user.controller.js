...
// Admin-only password reset: the caller sets a temporary password, we flip must_change_password
// so the next login is trapped on /change-password (ProtectedRoute + blockIfMustChangePassword).
...
export const resetUserPassword = asyncHandler(async (req, res) => {
  ...
  await bumpRefreshTokenVersion(target.id);
  res.json({ ok: true });
});
