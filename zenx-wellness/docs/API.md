...
Transport is Resend in production (`RESEND_API_KEY` required, checked at boot) and the console/file
transport (`server/.local/emails/`) by default everywhere else. Development can opt in to real
sending with `EMAIL_TRANSPORT=resend` (still requires `RESEND_API_KEY`); `NODE_ENV=test` refuses any
transport but `console` at startup, so a test run can never reach a mail provider. Manual test
...
| GET | `/insights/dietitian-overview` | dietitian | `{todaysAppointments[], attentionItems[], clientMomentum, stats: {clients: {total, changePct, addedThisWindow, addedPrevWindow, windowDays}, appointmentsToday: {total, changePct, sameDayLastWeek}, activePlans: {total, changePct, publishedThisWindow, publishedPrevWindow, windowDays}}, progressSeries: [{date, logs}] (last 7 days, zero-filled), planBreakdown: {active, completed, draft}}`. Every `changePct` is `null` when its baseline is zero — an account with no history shows no trend rather than an invented one. `planBreakdown` is derived from `published` + whether the plan's week has ended (`plans` has no status column), so the three buckets are exhaustive and mutually exclusive |
...
- `attentionItems` on `/insights/dietitian-overview` still returns an empty placeholder — nothing
  populates or consumes it. The rest of that response (stat cards with change figures, the 7-day
  progress series, and the plan breakdown) is real aggregation, rendered by
  `DietitianOverviewScreen.jsx`. `admin-overview`'s `growthSeries` is real as of Phase 8.
- `/insights/dietitian-overview`'s day boundaries come from the server's own clock
  (`startOfDay`/`endOfDay`), not the dietitian's stored `timezone` — so "today's appointments" and
  the progress series' last bucket follow the server's day, not the viewer's. Consistent with
  `admin-overview`, which has the same limitation; a per-user timezone version would need both
  endpoints changed together.
