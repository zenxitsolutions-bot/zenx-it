...
export function MobileNav() {
  const { user } = useAuth();
  const items = portalItemsFor(user).slice(0, MAX_TABS);
  const canMessage = user.role === 'client' || hasPermission(user, 'messages.use');
  const { data: unread } = useUnreadMessageCount(canMessage && (user.role === 'client' || user.role === 'dietitian'));
  const { data: supportUnread } = useSupportUnreadCount(canMessage && (user.role === 'dietitian' || user.role === 'admin'));
  const unreadCount =
  ...
  return (
    <nav
      ...
      className="sticky bottom-0 z-30 border-t border-line bg-white/90 pb-[env(safe-area-inset-bottom)] backdrop-blur min-[1050px]:hidden"
    >
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex h-15 flex-col items-center justify-center gap-1 px-1 text-[11px] transition-colors ${
                  isActive ? 'font-semibold text-coral' : 'text-muted-foreground'
                }`
              }
            >
              {({ isActive }) => (
                <>
                  <span
                    className={`relative grid h-7 w-12 place-items-center rounded-lg transition-colors ${
                      isActive ? 'bg-coral text-white shadow-sm' : 'text-muted-foreground'
                    }`}
                  >
