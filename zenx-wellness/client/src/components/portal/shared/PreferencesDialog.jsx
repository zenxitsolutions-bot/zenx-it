...
export function PreferencesDialog({ open, onOpenChange }) {
  ...
  function save() {
    ...
    if (isDietitian) patch.phone = values.phone || '';
    if (isClient) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={isClient || isDietitian ? 'max-h-[90dvh] overflow-y-auto sm:max-w-md' : 'sm:max-w-sm'}>
        <DialogHeader>
        ...
        {isDietitian && (
          <div className="grid gap-4">
            ...
            <div className="grid gap-1.5">
              <label className="text-sm font-medium text-forest" htmlFor="dietitian-account-phone">Phone number</label>
              <PhoneInput
                id="dietitian-account-phone"
                value={values.phone}
