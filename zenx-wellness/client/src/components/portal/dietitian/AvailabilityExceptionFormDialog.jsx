...
export function AvailabilityExceptionFormDialog({ open, onOpenChange }) {
  ...
  function onSubmit(values) {
    createException.mutate(
      {
        kind: values.kind,
        startAt: zonedTimeToUtcIso(values.startAt, timezone),
        endAt: zonedTimeToUtcIso(values.endAt, timezone),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Use "Blocked" for a day off, a break, or a holiday — "Extra hours" to open up a normally closed time. Times
            are in your schedule timezone ({timezone}).
          </DialogDescription>
        </DialogHeader>
