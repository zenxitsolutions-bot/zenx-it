...
// A plan's `week` is the UTC calendar date its Monday falls on (see client/src/lib/planBuilder.js
// for why this must stay UTC-safe) — a meal's own calendar date is that plus its weekday's offset.
function dateForMeal(plan, day) {
  const offset = WEEKDAYS.indexOf(day);
  if (offset < 0) return null;
  const base = new Date(plan.week);
...
export function getRecentMeals(plans, days = 15, now = new Date()) {
  ...
  const windowEnd = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 23, 59, 59, 999));
  ...
  for (const plan of plans ?? []) {
    for (const meal of plan.meals ?? []) {
      ...
      entries.push({ ...meal, date: date.toISOString().slice(0, 10), planId: plan._id });
    }
  }
