...
import { mysqlUri, secretEnv, assertDistinctSecrets, parseTrustProxy, originList } from './security.js';

export const env = {
  nodeEnv: process.env.NODE_ENV || 'development',
  port: Number(process.env.PORT || 4001),
  mysqlUrl: mysqlUri('MYSQL_URL', 'mysql://root:root@127.0.0.1:3307/zenx_admin'),
  trustProxy: parseTrustProxy(process.env.TRUST_PROXY),

  adminJwtAccessSecret: secretEnv('ADMIN_JWT_ACCESS_SECRET', 'dev-admin-access-secret-change-me'),
  adminJwtRefreshSecret: secretEnv('ADMIN_JWT_REFRESH_SECRET', 'dev-admin-refresh-secret-change-me'),
  adminJwtAccessTtl: process.env.ADMIN_JWT_ACCESS_TTL || '15m',
  adminJwtRefreshTtl: process.env.ADMIN_JWT_REFRESH_TTL || '30d',

  customerJwtAccessSecret: secretEnv('CUSTOMER_JWT_ACCESS_SECRET', 'dev-customer-access-secret-change-me'),
  customerJwtRefreshSecret: secretEnv('CUSTOMER_JWT_REFRESH_SECRET', 'dev-customer-refresh-secret-change-me'),
  ...
  clientOrigins: originList('CLIENT_ORIGINS', 'http://localhost:5174,http://localhost:5175,http://localhost:5173'),
  ...
  // still never sends real mail by accident, matching wellness-app's resolveTransportKind().
  emailTransport: process.env.EMAIL_TRANSPORT || '',
  resendApiKey: process.env.RESEND_API_KEY || '',
  emailFrom: process.env.EMAIL_FROM || 'ZenX Admin <onboarding@resend.dev>',

  // SMTP transport. `secure` means implicit TLS on connect (port 465); port 587 leaves it false
  // and upgrades via STARTTLS instead, which is what most providers want — so it's derived from
  // the port rather than defaulted to a constant that would be wrong half the time.
  ...
  wellnessMysqlUrl: mysqlUri('WELLNESS_MYSQL_URL', '', { required: false }),
...
assertDistinctSecrets([env.adminJwtAccessSecret, env.adminJwtRefreshSecret, env.customerJwtAccessSecret, env.customerJwtRefreshSecret]);
if (env.zenxDietitianHandoffSecret) env.zenxDietitianHandoffSecret = secretEnv('ZENX_DIETITIAN_HANDOFF_SECRET');
