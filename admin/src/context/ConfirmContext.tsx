...
export function ConfirmProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ConfirmContext.Provider value={{ confirm }}>
      {children}
      {state && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-offwhite/40 backdrop-blur-sm px-4">
          <div className="w-full max-w-sm rounded-xl2 border border-border bg-panel p-6 shadow-panel animate-fadeIn">
            <div
              className={`mb-4 flex h-10 w-10 items-center justify-center rounded-full ${
                state.options.danger ? "bg-danger/10 text-dangerInk" : "bg-lime/10 text-lime"
              }`}
            >
              ...
              <button
                ...
                className={`rounded-md px-4 py-2 text-sm font-semibold transition ${
                  state.options.danger
                    ...
                    : "bg-lime text-white hover:brightness-110"
                }`}
              >
